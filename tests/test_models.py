...
def test_daily_mean(test_data, test_index, test_columns,
    ...
    """ Test mean function works with zero and positive integers """
...
def test_normalize(test_data, test_index, test_columns, expected_data, expected_index, expected_columns):
    """Test normalisation works for arrays of one and positive integers.
       Assumption that test accuracy of two decimal places is sufficient."""
    from catchment.models import data_normalize
    pdt.assert_frame_equal(data_normalize(pd.DataFrame(data=test_data, index=test_index, columns=test_columns)),
                           pd.DataFrame(data=expected_data, index=expected_index, columns=expected_columns),
                           atol=1e-2)
